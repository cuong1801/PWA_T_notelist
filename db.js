                        ...
                        db.collection("notelist").where('category', '==', category_selected).orderBy("timepost", "desc").get().then(snapshot => {
                            snapshot.docs.forEach(doc => {
                                ...
                                if (doc.data().userID == idUser) {
                                    ...
                                    $("#name_notes").append(
                                        '<a  class="card3 a "  >' +
                                        '<div id="deletenote" class="go-corner" >' +
                                        '<div id="' + doc.id + '" class="go-arrow btn btn-outline-danger"  >' +
                                        'x' +
                                        '</div>' +
                                        '</div>' +
                                        '<div  data-toggle="modal" data-target="#exampleModalScrollable' + doc.id + '">' +
                                        ...
                                        '<button  id="' + doc.id + '" type="button" class="btn btn-outline-success border border-warning" style="width:100px; margin:30px 88px" >More</button>' +
                                        '</div>' +
